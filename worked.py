...
matrix1 = [[f"  {chr(96 + j)}  " if i == 0 else (str(i) if j == 0 else "| O |") for j in range(6 + 1)] for i in
           range(6 + 1)]
matrix1[0][0] = ""

matrix2 = [[f"  {chr(96 + j)}  " if i == 0 else (str(i) if j == 0 else "| 0 |") for j in range(6 + 1)] for i in
           range(6 + 1)]
matrix2[0][0] = ""
        ...
        for fr_point in fp:
            for sh_dot in item.dots:
                if abs(int(sh_dot[0]) - int(fr_point[0])) == 1 and abs(int(sh_dot[1]) - int(fr_point[1])) == 1 \
                        ...
                        or abs(int(sh_dot[0]) - int(fr_point[0])) == 0 and abs(
                    int(sh_dot[1]) - int(fr_point[1])) == 1:  # \
